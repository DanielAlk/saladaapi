class ProductsController < ApplicationController
  ...
  private
    ...
    def set_interaction_products
      ...
      @products = Product.distinct.select('products.*, MAX(interactions.updated_at) as updated_at, interactions.id as interaction_id').joins(:interactions).where(where_clause => user_id).group(:product_id).order('updated_at DESC')
    end
